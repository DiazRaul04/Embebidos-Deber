 ...
 * DEBER 5.3_SE
 * NOMBRE: Díaz Raúl
 * TEMA:  Desactivar funciones en RTOS
 * Instrucciones:     CÓDIGO 26
 */
...
void setup() { 
  Serial.begin(9600); //crear las rutinas de cada tarea 
...
void redLedControllerTask(void *pvParameters){//tarea 1
  pinMode(RED,OUTPUT);
  while(1){ //Serial.println("Proceso led rojo"); 
    redLEDProfiler++; 
...
void blueLedControllerTask (void *pvParameters){
  pinMode(BLUE,OUTPUT); 
  while(1){ //Serial.println("Proceso led azul"); 
    blueLEDProfiler++; 
...
void yellowLedControllerTask (void *pvParameters){
  ...
  while(1){ //Serial.println("Proceso led amarillo"); 
    yellowLEDProfiler++;
    Serial.print("YELLOW TASK:");
    Serial.println(yellowLEDProfiler); 
    digitalWrite(YELLOW,digitalRead(YELLOW)^1);
// vTaskPrioritySet(red_Handle,2);
// modificar prioridad de la rutina de led rojo no debe haber delays
monitor_suspend++;
